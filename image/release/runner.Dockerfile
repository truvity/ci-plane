# The RELEASE image for the GitHub Actions runner.
#
# IT CONTAINS NO `RUN`, AND THAT IS THE WHOLE POINT.
#
# Every instruction that has to EXECUTE lives in image/runner/Dockerfile,
...
# could not install").
ARG BASE_IMAGE=ghcr.io/truvity/ci-plane/runner-base
ARG BASE_TAG=latest
...
LABEL org.opencontainers.image.title="the GitHub Actions runner" \
